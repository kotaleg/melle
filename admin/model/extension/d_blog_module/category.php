...
// MySQL Hierarchical Data Closure Table Pattern
        ...
        if (isset($data['seo_url']) && !empty($data['seo_url'])) {
            ...
            $this->db->query("DELETE FROM " . DB_PREFIX . "seo_url
                WHERE query = 'bm_category_id=" . (int) $category_id . "'");
            $this->db->query("DELETE FROM " . DB_PREFIX . "seo_url
                WHERE keyword = '" . $this->db->escape($data['seo_url']) . "'");
...
// Fix for records with no paths
    ...
    public function getPostUrl($category_id)
    {
        $query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "seo_url
            WHERE query = 'bm_category_id=" . (int) $category_id . "'");
