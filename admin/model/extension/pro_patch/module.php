 ...
 *
 */
class ModelExtensionProPatchModule extends Model
{
    public function addModule($code, $data)
    {
        if (VERSION >= '2.1.0.0') {
            ...
            $this->db->query("INSERT INTO `" . DB_PREFIX . "module`
                SET `name` = '" . $this->db->escape($data['name']) . "',
                `code` = '" . $this->db->escape($code) . "',
                `setting` = '" . $this->db->escape(serialize($data)) . "'");
        }
    ...
    public function getLastIdByCode($code)
    {
        $q = $this->db->query("SELECT MAX(module_id) as lastId
