<?php
//калькулятор для доставки часть 1 начало
...
//калькулятор для доставки часть 1 конец
class ControllerSaleOrderdostavim extends Controller {
	private $error = array();
//калькулятор для доставки часть 2 начало
...
//калькулятор для доставки часть 2 конец
//здесь мы получаем guid города и guid региона
function GetGuidCityAndRegion($partName)
	{
		$url = "https://api.dostav.im/Address/City/?";
		$curl = curl_init();
		curl_setopt_array($curl, array(
		CURLOPT_URL => $url."partName=".urlencode($partName),
    ...
    function getSettingsDostavim($key) {
			...
			if ($err) {
                ...
                //condole.log(data.deliveryMethods[i].used);
                //condole.log(data.deliveryMethods[i].number);
                //condole.log(data.deliveryMethods[i].name);
                //condole.log(data.deliveryMethods[i].description);
//здесь мы получаем список вес из ЛК 	
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			'text' => 'Доставка Dostav.im',//$this->language->get('heading_title'),
			'href' => $this->url->link('sale/orderdostavim', 'user_token=' . $this->session->data['user_token'] . $url, true)
		);
		...
		foreach ($results as $result) {
			$data['orders'][] = array(
				...
				'view'          => $this->url->link('sale/orderdostavim/info', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . $result['order_id'] . $url, true),
		...
		if ($order_info) {
			...
			foreach ($products as $product) {
				...
				$data['products'][] = array(
					...
					'totaldostavim'   => $product['total'] + ($this->config->get('config_tax') ? ($product['tax'] * $product['quantity']) : 0),
					'href'     		   => $this->url->link('catalog/product/edit', 'user_token=' . $this->session->data['user_token'] . '&product_id=' . $product['product_id'], true)
				);
				
				$prod_info[$c] = $this->model_catalog_product->getProduct($product['product_id']);
				$prod_count[$c] = $product['quantity'];				
				$c++;
        ...
        //2019
			...
			$data['productprice'] = 0;
				foreach ($data['products'] as $product) {
					$data['productname'] .=  $product['name'].$product['quantity']. ' шт, ';
					$data['productprice'] =  number_format($product['totaldostavim'],2,'.','')+$data['productprice'];
					//$productprice = $product['totaldostavim'];
					//print_r($data['productprice']);
				} 
        //2019				
			

			$data['sumprise'] = $data['products'];	
			...
			$param1 =  $tokenDostavim;
			$data['tokenDostavim'] = $param1;
			//$dostavimlength = $prod_info['length'];						
			//$dostavimwidth = $prod_info['weight'];						
			//$dostavimheight = $prod_info['height'];						
			//$dostavimweight = $prod_info['weight'];	
			//print_r($prod_info);
			//получаем токен
			//калькулятор доставки часть 4	
			$weightDostavim = 0;
			$variableOrderCost = 0;
			$productDostavim = '';
		    ...
		    $parametrs = $this->GetListParametrs($tokenDostavim);   //2019
            $parametrsLength = $parametrs->deliveryMethods[3]->length;	   //2019
            $parametrsWidth = $parametrs->deliveryMethods[3]->width;   //2019	
            $parametrsHeight = $parametrs->deliveryMethods[3]->height;   //2019	
            $parametrsWeigh = $parametrs->deliveryMethods[3]->weight;   //2019	
		
//mail			
			$numberBlock = $this->getSettingsDostavim($tokenDostavim);
			
            $settingName3 = $numberBlock->deliveryMethods[3]->name;	
            $settingNumber3 = $numberBlock->deliveryMethods[3]->number;				
            $settingName4 = $numberBlock->deliveryMethods[4]->name;	
            $settingNumber4 = $numberBlock->deliveryMethods[4]->number;
			
            $data['settingName3'] = $settingName3;
            $data['settingNumber3'] = $settingNumber3;
            $data['settingName4'] = $settingName4;
            $data['settingNumber4'] = $settingNumber4;
//mail			

			
			//2019 функционал
		    $limit = explode("::", $data['adressdostavim']);
		    if (isset($limit[1])){
			$deliveryAddressGet = explode("::", $data['adressdostavim']);
			$data['deliveryAddressParce'] = $deliveryAddressGet[0];			
			$data['deliveryService'] = $deliveryAddressGet[1];
			$data['deliveryId'] = $deliveryAddressGet[2];
			$data['deliveryNum'] = $deliveryAddressGet[3];
		    }
		      else
			{
			   if (trim(stristr(strip_tags($data['shipping_method']), ' -', true)) == trim($settingName4))
			   {
				$data['deliveryService'] = 'Почта России';
				$data['deliveryAddressParce'] = $data['adressdostavim']; //4 свой курьер 
				$data['deliveryNum'] = 5;				   
			   }
			   else
			   {
				$data['deliveryService'] = 'Курьерская служба';
				$data['deliveryAddressParce'] = $data['adressdostavim']; //4 свой курьер 
				$data['deliveryNum'] = 4;			   
			   }
			}
			...
			$data['guidDostavim'] = $guidDostavim[0]->guid;	
		    
		    
			foreach ($prod_info as $i => $item) {


				for($j=0;$j < (int)$prod_count[$i];$j++){
			...
			//print_r($objectHeighWidthlength);			
			if($weightDostavim<100)
				$weightDostavim=100;
			
			if (isset($objectHeighWidthlength)){		
				$dostlength = ceil($objectHeighWidthlength->lenght);
				$dostwidth = ceil($objectHeighWidthlength->width);
				$dostheight = ceil($objectHeighWidthlength->height);		
				$dostweight = $weightDostavim;
			}
			...
			$data['tokenDostavim'] = $param1;
			$data['dostavimlength'] = $dostlength;						
			$data['dostavimwidth'] = $dostwidth;						
			$data['dostavimheight'] = $dostheight;						
			$data['dostavimweight'] = $dostweight;	
			//print_r($prod_info);		
			...
			$data['sumdelivery'] = number_format($totals[1]['value'],2,'.','');
			
			$data['namedelivery'] = $totals[1]['title'];	
			...
			$data['myaccept_language'] = $data['accept_language'][0];
			...
			$this->response->setOutput($this->load->view('sale/order_infodostavim', $data));
