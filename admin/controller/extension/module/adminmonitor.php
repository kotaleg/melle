...
class ControllerExtensionModuleAdminMonitor extends Controller {
    ...
    public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            // if (!empty($this->request->post['OaXRyb1BhY2sgLSBDb21'])) {
                $this->request->post['adminmonitor']['LicensedOn'] = 'Ivan';
            // }

            // if (!empty($this->request->post['cHRpbWl6YXRpb24ef4fe'])) {
                $this->request->post['adminmonitor']['License'] = json_decode(array('licenseExpireDate' => '31.01.2039 14:05:08'), true);
            // }
            ...
            $this->response->redirect($this->url->link($this->config->get('adminmonitor_module_path'), 'user_token=' . $this->session->data['user_token'], 'SSL'));
        }
        ...
        $groups = $this->{$this->config->get('adminmonitor_model_key')}->getEventGroups();
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link($this->config->get('adminmonitor_extension_route'), 'user_token=' . $this->session->data['user_token'], 'SSL')
        );

        $data['breadcrumbs'][] = array(
            'text' => $this->language->get('heading_title'),
            'href' => $this->url->link($this->config->get('adminmonitor_module_path'), 'user_token=' . $this->session->data['user_token'], 'SSL')
        );

        $data['action'] = $this->url->link($this->config->get('adminmonitor_module_path'), 'user_token=' . $this->session->data['user_token'], 'SSL');

        $data['cancel'] = $this->url->link($this->config->get('adminmonitor_extension_route'), 'user_token=' . $this->session->data['user_token'], 'SSL');

        $data['rehook'] = $this->url->link($this->config->get('adminmonitor_module_path') . '/rehook_events', 'user_token=' . $this->session->data['user_token'], 'SSL');
        ...
        $data['license_encoded'] = !empty($data['setting']['adminmonitor']['LicensedOn']) ? base64_encode(json_encode($data['setting']['adminmonitor']['License'])) : '';
        $data['license_expire_date'] = !empty($data['setting']['adminmonitor']['LicensedOn']) ? date("F j, Y", strtotime($data['setting']['adminmonitor']['License']['licenseExpireDate'])) : '';
        $data['open_support_ticket_link'] = 'http://isenselabs.com/tickets/open/' . base64_encode('Support Request') . '/' . base64_encode('179') . '/' . base64_encode($_SERVER['SERVER_NAME']);

        $data['header'] = $this->load->controller('common/header');
        $data['column_left'] = $this->load->controller('common/column_left');
        $data['footer'] = $this->load->controller('common/footer');

        $data['tabs'] = $this->getTabs($data);

        $this->response->setOutput($this->load->view($this->config->get('adminmonitor_module_path'), $data));
    }
    ...
    public function __call($function, $arg_raw) {
        ...
        foreach ($events as $event) {
            ...
            if (!empty($arg) || $hook == 'admin_post_setting_edit') {
                if (eval($event['eval']) === false) {
                    return;
                }
            }
    ...
    public function list_events() {
        ...
        foreach ($results as $result) {
            ...
            $output['events'][] = array(
                ...
                'message' =>  sprintf($this->language->get('message_' . $result['event_type']), $this->language->get('subject_prefix_' . $result['event_group']), str_replace('{user_token}', $this->session->data['user_token'], $result['subject'])),
    ...
    public function loginSetup() {
        $this->session->data['adminmonitor_login_detect'] = 1;
    }

    public function loginCatch() {
        $this->event->unregister("controller/*/before", $this->config->get('adminmonitor_module_path') . "/loginCatch");

        if (!empty($this->request->get['route']) && $this->request->get['route'] != 'common/login' && !empty($this->session->data['adminmonitor_login_detect'])) {
            unset($this->session->data['adminmonitor_login_detect']);
            $this->load->model($this->config->get('adminmonitor_module_path'));
            $this->{$this->config->get('adminmonitor_model_key')}->logEvent(array(
              'user_id' => $this->user->getId(),
              'user_name' => $this->user->getUserName(),
    ...
    public function logoutCatch() {
        ...
        $this->{$this->config->get('adminmonitor_model_key')}->logEvent(array(
          ...
          'event_type' => 'logout',
          'event_group' => 'logout',
          'argument_hook' => 'custom_logout',
          'data' => '',
          'subject' => ''
        ));
    }

    public function productManagerHandler($eventRoute, $data) {
        if (!empty($this->request->post['pid'])) {
            $product_id = $this->request->post['pid'];

            $pids = array();
            if (basename($eventRoute) == 'updateproduct') {
                $pids[] = $product_id;
            } else {
            ...
            foreach ($events as $event) {
                if ($event['argument_hook'] != $hook) continue;
                $subjectGenerator = $event['eval'];
                break;
            }

            foreach ($pids as $pid) {
                $arg = $pid;
                eval($subjectGenerator);
    ...
    public function setUserIdCookie() {
        setcookie('amui', $this->user->getId(), time() + 86400, '/');
    }
    ...
    private function getTabs($data) {
        ...
        foreach ($files as $file) {
            ...
            $result[] = array(
                'html' => $this->load->view($this->config->get('adminmonitor_module_path') . '/' . $name_map[$file]['id'], $data),
