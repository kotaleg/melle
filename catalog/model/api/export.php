...
class ModelApiExport extends Model
{
    ...
    public function actionYandexOffersAllImagesExport()
    {
        $file = $this->export_path . 'yandex-offers-all-images.xml';
        if (is_file($file)) {
            @unlink($file);
        }
        $this->createPath($file);
        ...
        foreach ($products as $product) {
            ...
            if ($product['image'] && is_file(DIR_IMAGE . $product['image'])) {
                $images[] = $this->model_tool_image->resize($product['image'], $this->imageWidth, $this->imageHeight);
            }
            ...
            foreach ($combinations as $c) {
                ...
                if (isset($c['image']) && is_file(DIR_IMAGE . $c['image'])) {
                    $images[] = $this->model_tool_image->resize($c['image'], $this->imageWidth, $this->imageHeight);
                }
            }

            if (empty($images)) {
                $images[] = $this->model_tool_image->resize('image/placeholder.png', $this->imageWidth, $this->imageHeight);
            }
