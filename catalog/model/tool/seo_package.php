...
class ModelToolSeoPackage extends Model {
  ...
  public function hrefLang() {
    ...
    foreach ($langs as $lang) {
      ...
      if ($route == 'common/home' && $this->config->get('mlseo_default_lang') == $lang['code']) {
        $hreflangs[] = array(
          'href' => !empty($lang_to_store) ? str_replace(array(rtrim($this->config->get('config_url'),'/'), rtrim($this->config->get('config_ssl'),'/')), array_map(array('self','trimSlash'), $lang_to_store[$lang['code']]), str_replace('/'.$lang['code'].'/', '/', $this->url->link($route, $url))) : str_replace('/'.$lang['code'].'/', '/', $this->url->link($route, $url)),
          'hreflang' => 'x-default'
        );
      }
  ...
  public function getSeoDescription($type, $item_id) {
    $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "seo_".$this->db->escape($type)."_description d WHERE ".$this->db->escape($type)."_id = '" . (int)$item_id . "' AND store_id = '".(int) $this->config->get('config_store_id')."' AND language_id = '".(int) $this->config->get('config_language_id')."'")->row;
  ...
  public function microdataJson($page, $data) {
    ...
    if ($page == 'product' && !empty($config['product'])) {
      ...
      if ($data['product_info']['quantity'] > 0) {
        ...
        // @todo: make option to display out of stock or not
        if (false) {
  ...
  public function addUrl404($url) {
    ...
    if (isset($query['count'])) {
      $this->db->query("UPDATE " . DB_PREFIX . "url_404 SET count = '".(int)($query['count']+1)."', date_accessed = NOW() WHERE url_404_id = '". $this->db->escape($query['url_404_id']) . "'");
    } else {
