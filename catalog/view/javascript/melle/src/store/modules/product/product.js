...
const getters = {
  ...
  getActiveOptions: (state) => {
    ...
    state.options.forEach((option) => {
      if (
        !isArray(option.product_option_value) ||
  ...
  getOptionsForCart: (state) => {
    ...
    state.options.forEach((option) => {
      ...
      option.product_option_value.forEach((option_value) => {
        if (option_value.selected === true) {
          options[toString(option.product_option_id)] = toString(
              ...
              if (!isEqual(one, req_)) {
                ...
                if (real_keys.o_key !== false && real_keys.ov_key !== false) {
                  commit('setOptionSelectStatus', {
                    o_key: real_keys.o_key,
                    ov_key: real_keys.ov_key,
                    status: true,
                  })
                }
              }
...
const mutations = {
  ...
  setOptionSelectStatus(state, { o_key, ov_key, status }) {
    Vue.set(
      state.options[o_key]['product_option_value'][ov_key],
      'selected',
      status
    )
  },
