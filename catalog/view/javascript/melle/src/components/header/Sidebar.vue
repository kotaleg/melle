<template>
  <section class="sidebar-popup" :style="sidebarStyle">
    <div
      ...
      :class="['sidebar-popup__content', { nameClass }]"
...
export default {
  ...
  computed: {
    ...
    sidebarStyle() {
      let styles = {
        display: 'block',
        backgroundColor: this.sidebar_opened
          ? 'rgba(0, 0, 0, .5)'
          : 'rgba(0, 0, 0, 0)',
      }
      return styles
    ...
    sidebarContentStyle() {
      let styles = {}
      styles[this.position] = this.sidebar_opened
        ? '0px'
        : `-${this.sidebarPopupContentWidth + 50}px`
      return styles
    },
    sidebarButtonStyle() {
      let styles = {
      ...
      styles[this.position] =
        this.windowWidth > 767 ? 'calc(100% - 52px)' : 'calc(100% - 40px)'
      return styles
    },
  },
  methods: {
