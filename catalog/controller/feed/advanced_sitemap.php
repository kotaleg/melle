...
class ControllerFeedAdvancedSitemap extends Controller {
      ...
      if ($items) {
        while ($items > 0) {
          ...
          if (!empty($this->request->get['lang'])) {
            $output .= '    <loc><![CDATA[' . html_entity_decode($this->url->link('feed/advanced_sitemap/'.$type, 'lang='.$this->request->get['lang'].($page ? '&page='.$page++ : ''), $this->isSSL)).']]></loc>';
          } else {
            $output .= '    <loc><![CDATA[' . html_entity_decode($this->url->link('feed/advanced_sitemap/'.$type, ($page ? 'page='.$page++ : ''), $this->isSSL)).']]></loc>';
          }
  ...
  public function generate($type) {
    ...
    foreach ($items as $item) {
      ...
      if ($type == 'category') {
        ...
        $output .= '<loc><![CDATA[' .html_entity_decode($this->url->link('product/category', 'path='. ($path ? $path . '_' : '') . $item['category_id'])). ']]></loc>';
      } else if ($type == 'manufacturer') {
        $output .= '<loc><![CDATA[' .html_entity_decode($this->url->link('product/manufacturer/info', 'manufacturer_id=' . $item['manufacturer_id'])). ']]></loc>';
      } else if ($type == 'information') {
        $output .= '<loc><![CDATA[' .html_entity_decode($this->url->link('information/information', 'information_id=' . $item['information_id'])). ']]></loc>';
      } else {
        $output .= '<loc><![CDATA[' .html_entity_decode($this->url->link('product/'.$type, $type.'_id=' . $item[$type.'_id'])). ']]></loc>';
      }
  ...
  private function get_product($data, $count = false) {
    ...
    if (1) {
      ...
      $rows = $this->db->query("SELECT DISTINCT p.product_id, pd.name AS name, p.image FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'" . $extra_where . " LIMIT " . (int)$data['start'] . "," . (int)$data['limit'])->rows;

      foreach ($rows as $k => $row) {
        $rows[$k]['additional_images'] = array();
        $images = $this->db->query("SELECT image FROM " . DB_PREFIX . "product_image WHERE product_id = '" . (int)$row['product_id'] . "' ORDER BY sort_order ASC")->rows;
  ...
  private function generate_journal($data, $config) {
    ...
    $output .= '<url>';
    $output .= '<loc><![CDATA[' .html_entity_decode($this->url->link('journal2/blog', '')). ']]></loc>';
    ...
    foreach ($this->model_journal2_blog->getCategories() as $category) {
      $output .= '<url>';
      $output .= '<loc><![CDATA[' .html_entity_decode($this->url->link('journal2/blog', 'journal_blog_category_id=' . $category['category_id'])). ']]></loc>';
    ...
    foreach ($this->model_journal2_blog->getPosts() as $post) {
      $output .= '<url>';
      $output .= '<loc><![CDATA[' .$this->url->link('journal2/blog/post', 'journal_blog_post_id=' . $post['post_id']). ']]></loc>';
  ...
  private function fullCategoryPath($category) {
    ...
    while (!empty($category['parent_id'])) {
      ...
      $category = $this->db->query("SELECT parent_id FROM " . DB_PREFIX . "category WHERE category_id = '" . (int)$category['parent_id']. "'")->row;
    }
